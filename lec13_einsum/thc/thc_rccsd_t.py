...
class _energy_denominator_ijkabc(einsum_holder._einsum_term):
    def __init__(self):
        super().__init__(
            "ene_deno",
            "iT,jT,kT,aT,bT,cT->ijkabc",
            args=["TAUO", "TAUO", "TAUO", "TAUV", "TAUV", "TAUV"],
        )
...
def _apply_PL(W: einsum_holder._expr_holder):
    ...
    return (
        W
        + W.transpose((0, 1, 4, 5, 2, 3))
        + W.transpose((2, 3, 0, 1, 4, 5))
        + W.transpose((2, 3, 4, 5, 0, 1))
        + W.transpose((4, 5, 0, 1, 2, 3))
        + W.transpose((4, 5, 2, 3, 0, 1))
    )
...
def _rccsd_t_kernel(
    mycc=None,
    eris=None,
    t1: einsum_holder._expr_holder = None,
    t2: einsum_holder._expr_holder = None,
    verbose=logger.NOTE,
):
