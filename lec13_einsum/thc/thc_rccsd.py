...
def update_amps(
    cc,
    t1: einsum_holder._expr_holder,
    t2: einsum_holder._expr_holder,
    eris,
    ...
    t2new = (
        -t2new
    )  # the sign is due to the fact that ene_denominator is positive defniete
...
class _fake_eris_full(ccsd._ChemistsERIs):
    def __init__(self, fock, Xo, Xv, THC_INT):

        self.fock = fock
        self.mo_energy = np.diag(fock)
        self.ovvo = np.einsum(
            "iP,aP,PQ,jQ,bQ->iabj", Xo, Xv, THC_INT, Xo, Xv, optimize=True
        )
        self.oovv = np.einsum(
            "iP,jP,PQ,aQ,bQ->ijab", Xo, Xo, THC_INT, Xv, Xv, optimize=True
        )
        self.ovov = np.einsum(
            "iP,aP,PQ,jQ,bQ->iajb", Xo, Xv, THC_INT, Xo, Xv, optimize=True
        )
        self.ovoo = np.einsum(
            "iP,aP,PQ,jQ,kQ->iajk", Xo, Xv, THC_INT, Xo, Xo, optimize=True
        )
        self.ovvv = np.einsum(
            "iP,aP,PQ,bQ,cQ->iabc", Xo, Xv, THC_INT, Xv, Xv, optimize=True
        )
        self.ooov = np.einsum(
            "iP,jP,PQ,kQ,aQ->ijka", Xo, Xo, THC_INT, Xo, Xv, optimize=True
        )
        self.oooo = np.einsum(
            "iP,jP,PQ,kQ,lQ->ijkl", Xo, Xo, THC_INT, Xo, Xo, optimize=True
        )
        self.vvvv = np.einsum(
            "aP,bP,PQ,cQ,dQ->abcd", Xv, Xv, THC_INT, Xv, Xv, optimize=True
        )
...
if __name__ == "__main__":
    ...
    scheduler = einsum_holder.THC_scheduler(
        ...
        XV_T2=Xv_T2,
        PROJECTOR=PROJ,
        use_torch=False,  # you can try true if you have gpu
        with_gpu=False,  # you can try true if you have gpu
    )
    ...
    t1 = T1
    t2_full = np.einsum(
        "iP,aP,PQ,jQ,bQ->ijab", Xo_T2, Xv_T2, THC_T2, Xo_T2, Xv_T2, optimize=True
    )
    ...
    t2_new *= eijab
    eijab_new = np.einsum(
        "iP,jP,aP,bP->ijab", Tau_o, Tau_o, Tau_v, Tau_v, optimize=True
    )
    t2_new *= eijab_new

    t2_new_projected = np.einsum(
        "AP,iP,aP,ijab,jQ,bQ,QB->AB",
        PROJ,
        Xo_T2,
        Xv_T2,
        t2_new,
