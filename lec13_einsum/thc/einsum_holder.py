...
SUPPORTED_INPUT_NAME = [
    ...
    "TAUV",
    "THC_INT",
    "THC_T2",  ## NOTE: only these terms are involved in THC!
    "XO_T2",
    "XV_T2",
    "PROJ",  ## used in THC-CCSD, "PROJ" stands for PROJ_THC
    "PROJ_ROBUST",
    "PROJ_MP2",
    "PROJ_MP3",
    "R1",
...
class _einsum_term:
    ...
    def build(self):
        ...
        if self.is_skeleton:
            ...
            assert (
                nocc_indices + nvir_indices <= 4
            )  # only support 4 indices for skeleton
            self._dummy_indices = []
            ...
            self._dummy_indices = (
                self.einsum_str.split("->")[0].strip().replace(",", "")
            )
            self._dummy_indices = list(
                set([i for i in self._dummy_indices if i not in self._output_indices])
            )
    ...
    def _map_indices(self, _indices_map):  ### change the indices of the output tensor
        ...
        for d_k in dummy_indices_affected:
            if d_k in OCC_INDICES:
                for _id_ in OCC_INDICES:
                    if (
                        _id_ not in indices_map_real.values()
                        and _id_ != d_k
                        and _id_ not in self._dummy_indices
                    ):
        ...
        new_output_indices = [
            indices_map_real[i] if i in indices_map_real else i
            for i in self._output_indices
        ]
        new_einsum_str = [
            indices_map_real[i] if i in indices_map_real else i for i in self.einsum_str
        ]
    ...
    def _conj_scalar(cls, other):
        if isinstance(other, (int, float)):
            ...
            raise ValueError(
                "Conjugation with type {} not supported.".format(type(other))
            )
    ...
    def __mul__(self, other):
        if isinstance(other, (int, float, complex)):
            ...
            res = _einsum_term(
                self.name, self.einsum_str, self.factor * other, self.args
            )
            if self._conj:
                res = res.conj()
            return res
    ...
    def _build_path_numpy(self, scheduler, **kwargs):
        ...
        self._contract_fn = partial(
            np.einsum, subscripts=subscripts, optimize=contract_path[0]
        )
    ...
    def _build_path_opt_einsum(self, scheduler, **kwargs):
        ...
        if isinstance(optimize, bool):
            ...
            path = oe.contract_path(
                self.einsum_str, *tensors, memory_limit=memory, optimize=optimize
            )
...
class _thc_eri_oooo(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_oooo", "iP,jP,PQ,kQ,lQ->ijkl", args=["XO", "XO", "THC_INT", "XO", "XO"]
        )
...
class _thc_eri_ovoo(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_ovoo", "iP,aP,PQ,kQ,lQ->iakl", args=["XO", "XV", "THC_INT", "XO", "XO"]
        )
...
class _thc_eri_ovov(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_ovov", "iP,aP,PQ,jQ,bQ->iajb", args=["XO", "XV", "THC_INT", "XO", "XV"]
        )
...
class _thc_eri_ooov(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_ooov", "iP,jP,PQ,kQ,aQ->ijka", args=["XO", "XO", "THC_INT", "XO", "XV"]
        )
...
class _thc_eri_ovvv(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_ovvv", "iP,aP,PQ,bQ,cQ->iabc", args=["XO", "XV", "THC_INT", "XV", "XV"]
        )
...
class _thc_eri_ovvo(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_ovvo", "iP,aP,PQ,bQ,kQ->iabk", args=["XO", "XV", "THC_INT", "XV", "XO"]
        )
...
class _thc_eri_oovv(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_oovv", "iP,jP,PQ,aQ,bQ->ijab", args=["XO", "XO", "THC_INT", "XV", "XV"]
        )
...
class _thc_eri_vvvv(_einsum_term):
    def __init__(self):
        super().__init__(
            "eri_vvvv", "aP,bP,PQ,cQ,dQ->abcd", args=["XV", "XV", "THC_INT", "XV", "XV"]
        )
...
class _energy_denominator(_einsum_term):
    def __init__(self):
        super().__init__(
            "ene_deno", "iT,jT,aT,bT->ijab", args=["TAUO", "TAUO", "TAUV", "TAUV"]
        )
...
class _expr_mp2_t2(_einsum_term):
    def __init__(self):
        super().__init__(
            "MP2_T2",
            "iP,aP,PQ,jQ,bQ,iT,jT,aT,bT->iajb",
            args=["XO", "XV", "THC_INT", "XO", "XV", "TAUO", "TAUO", "TAUV", "TAUV"],
        )
...
class _expr_t2(_einsum_term):
    def __init__(self):
        super().__init__(
            "T2",
            "iP,aP,PQ,jQ,bQ->iajb",
            args=["XO_T2", "XV_T2", "THC_T2", "XO_T2", "XV_T2"],
        )
...
class _expr_holder:
    ...
    def _map_indices(self, indices_map):
        ...
        if isinstance(indices_map, list):
            ...
            self.indices = [
                indices_map[i] if i in indices_map else i for i in self.indices
            ]
    ...
    def build_contraction_path(self, scheduler, backend, **kwargs):
        ...
        if backend != "cotengra":
            ...
            if scheduler.with_gpu is False:
                ...
                process_term_with_fixed_args = partial(
                    self.process_term, scheduler=scheduler, backend=backend, **kwargs
                )
                terms = None
                with Pool() as pool:
        ...
        print(
            "build contraction %s with time %.2f %.2f "
            % (self.name, t2[0] - t1[0], t2[1] - t1[1])
        )
    ...
    def contract(self, scheduler):
        ...
        for term in self.terms:
            ...
            print(
                "contraction %s with time %.2f %.2f "
                % (term.name, t2[0] - t1[0], t2[1] - t1[1])
            )
...
def thc_einsum_sybolic(subscripts, *tensors, **kwargs):
    ...
    einsum_term = _einsum_term(
        name=name, einsum_str=subscripts, factor=1.0, args=tensors_indx_aligned
    )
        ...
        for term in terms_now:
            ...
            if term.is_parsed:
                ...
                for tr_script, arg in zip(subscripts, term.args):
                    ...
                    if term_parse_found:
                        ...
                        if isinstance(arg, str):
                            ...
                            assert isinstance(arg, _einsum_term) or isinstance(
                                arg, _expr_holder
                            )
                            if isinstance(arg, _einsum_term):
                                ...
                                for sub_tr_script, sub_arg in zip(
                                    arg_LHS, arg_to_add.args
                                ):
                                ...
                                if arg.is_cached:
                                    arg_to_add = copy.deepcopy(arg)
                                    arg_to_add._map_indices(list(tr_script))
                                    scheduler.register_intermediates(
                                        arg.name, arg_to_add
                                    )
                                    tensor_scripts += tr_script + ","
                                    ...
                                    parsed_arg_to_add, _ = _parse_expression(
                                        arg_to_add, scheduler
                                    )
                                    ...
                                    for sub_term in parsed_arg_to_add.terms:
                                        sub_term_to_add = copy.deepcopy(sub_term)
                                        sub_term_to_add._relabel_dummy_indices(
                                            indices_forbidden
                                        )
                                        arg_LHS = sub_term_to_add.einsum_str.split(
                                            "->"
                                        )[0].split(",")
                                        args_to_add = copy.deepcopy(args)
                                        tensor_scripts_to_add = tensor_scripts
                                        for sub_tr_script, sub_arg in zip(
                                            arg_LHS, sub_term_to_add.args
                                        ):
                                            tensor_scripts_to_add += sub_tr_script + ","
                                            args_to_add.append(sub_arg)
                                        tensor_scripts_2.append(tensor_scripts_to_add)
                ...
                for _id_ in range(len(tensor_scripts_2)):
                    ...
                    if factor == 0.0:
                        continue
                    new_expr = _einsum_term(
                        name=NAME_FORMAT % n_term_parsed,
...
class THC_scheduler:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.proj_type in [
            THC_scheduler.thc_proj_name,
            THC_scheduler.thc_robust_proj_name,
        ]:
            self.init_projector()
            ...
            raise NotImplementedError(
                "Projector type %s not implemented." % self.proj_type
            )
        ...
        self._intermediates_hierarchy = (
            []
        )  # the first level determined by only the input tensors
    ...
    def __str__(self):
        ...
        for name, expr in zip(
            self._registered_intermediates_name, self._expr_intermediates
        ):
            res += "Intermediate {} : \n".format(name)
            res += str(expr)
        res += "\n"
    ...
    def init_projector(self, X_o=None, X_v=None):
        ...
        PROJ = THC_scheduler._evaluate_UDUT_mutliscale(
            U_RR, D_RR_inv
        )  # have serious numerical problem, but still cannot be fixed by this subroutine
        ...
        self.projector_inv_sqrt = (
            self.projector_inv_sqrt + self.projector_inv_sqrt.T
        ) / 2.0
        ...
        if self.use_torch:
            ...
            self.projector_inv_sqrt = _to_torch_tensor(
                self.projector_inv_sqrt, self.with_gpu
            )
    ...
    def init_mp2_projector(self, mp2_cutoff=1e-3, df_obj=None):
        ...
        for k, eri1 in enumerate(df_obj.loop()):
            ...
            L_cW_ia[p0:p1, :, :, :] = np.einsum(
                "iW,aW,Cia->CWia", tau_o, tau_v, Lov, optimize=True
            )
    ...
    def register_expr(self, name, expression):
        ...
        if name == THC_scheduler.t2_new_name:
            ...
            expression_new = thc_einsum_sybolic(
                "AP,iP,aP,%s,jQ,bQ,QB->AB" % indices_str,
                _proj_expr,
                _xo_expr,
                _xv_expr,
                expression,
    ...
    def get_tensor(self, name):
        ...
        if name in self._input_tensor_name:
            ...
            res = self._cached_intermediates[
                self._registered_intermediates_name.index(name)
            ]
            if res is None:
                # print("Evaluate intermediate: ", name)
                res = self._expr_intermediates[
                    self._registered_intermediates_name.index(name)
                ].contract(self)
    ...
    def _build_expression(self):
        ...
        for _id_ in range(len(self._expr_intermediates)):
            self._expr_intermediates[_id_], dep = _parse_expression(
                self._expr_intermediates[_id_], self
            )
        ...
        while True:
            ...
            for name in all_intermediates:
                ...
                if all(
                    [dep in removed_intermediates for dep in self._dependency[name]]
                ):
    ...
    def _build_contraction(self, backend, **kwargs):
        ...
        assert len(self._registered_intermediates_name) == len(self._expr_intermediates)
        assert len(self._cached_intermediates) == len(
            self._registered_intermediates_name
        )
        ...
        if self.use_torch:
            ...
            for name in self._input_tensor_name:
                ...
                if isinstance(
                    getattr(self, self.tensor_name_to_attr_name[name]), np.ndarray
                ):
                    setattr(
                        self,
                        self.tensor_name_to_attr_name[name],
                        _to_torch_tensor(
                            getattr(self, self.tensor_name_to_attr_name[name]),
                            self.with_gpu,
                        ),
                    )
        ...
        for i in range(NLAYER):
            for name in self._intermediates_hierarchy[i]:
                if name in self._registered_intermediates_name:
                    self._expr_intermediates[
                        self._registered_intermediates_name.index(name)
                    ].build_contraction_path(self, backend, **kwargs)
                else:
    ...
    def _evaluate(self, expr_name):
        ...
        if expr_name in self._input_tensor_name:
            ...
            self._cached_intermediates[
                self._registered_intermediates_name.index(expr_name)
            ] = None
            return self.get_tensor(expr_name)
        else:
    ...
    def evaluate_t1_t2(self, t1, thc_t2, evaluate_ene=True, only_ene=False):
        ...
        NAME = [
            THC_scheduler.t1_new_name,
            THC_scheduler.t2_new_name,
            THC_scheduler.ccsd_energy_name,
        ]
        ...
        if evaluate_ene:
            ...
            if self.use_torch:
                self._expr_cached[THC_scheduler.ccsd_energy_name] = self._expr_cached[
                    THC_scheduler.ccsd_energy_name
                ].item()
            if only_ene:
                ...
                return (
                    self._expr_cached[THC_scheduler.ccsd_energy_name],
                    self._expr_cached[THC_scheduler.t1_new_name],
                    self._expr_cached[THC_scheduler.t2_new_name],
                )
        else:
            if only_ene:
...
if __name__ == "__main__":
    ...
    scheduler_test = THC_scheduler(
        X_o, X_v, THC_INT, T1, X_o_T2, X_v_T2, THC_T2, Tau_o, Tau_v, Proj
    )
